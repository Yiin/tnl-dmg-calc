...
function minifyBuild(build: Build): any {
  ...
  // Critical stats
  if (build.meleeCritical && build.meleeCritical !== 1000) minified.mc = build.meleeCritical;
  if (build.rangedCritical && build.rangedCritical !== 1000) minified.rc = build.rangedCritical;
  if (build.magicCritical && build.magicCritical !== 1000) minified.mgc = build.magicCritical;
  if (build.criticalDamage && build.criticalDamage !== 50) minified.cd = build.criticalDamage;
  
  // Heavy attack
  if (build.meleeHeavyAttack && build.meleeHeavyAttack !== 500) minified.mh = build.meleeHeavyAttack;
  if (build.rangedHeavyAttack && build.rangedHeavyAttack !== 500) minified.rh = build.rangedHeavyAttack;
  if (build.magicHeavyAttack && build.magicHeavyAttack !== 500) minified.mgh = build.magicHeavyAttack;
  
  // Hit stats
  if (build.meleeHit && build.meleeHit !== 2000) minified.mhi = build.meleeHit;
  if (build.rangedHit && build.rangedHit !== 2000) minified.rhi = build.rangedHit;
  ...
  if (build.shieldBlockPenetrationChance) minified.sbp = build.shieldBlockPenetrationChance;
...
function minifyEnemy(enemy: Enemy): any {
  ...
  if (enemy.meleeDefense && enemy.meleeDefense !== 500) minified.md = enemy.meleeDefense;
  if (enemy.rangedDefense && enemy.rangedDefense !== 500) minified.rd = enemy.rangedDefense;
  if (enemy.magicDefense && enemy.magicDefense !== 500) minified.mgd = enemy.magicDefense;
  if (enemy.meleeEndurance && enemy.meleeEndurance !== 1000) minified.me = enemy.meleeEndurance;
  if (enemy.rangedEndurance && enemy.rangedEndurance !== 1000) minified.re = enemy.rangedEndurance;
  if (enemy.magicEndurance && enemy.magicEndurance !== 1000) minified.mge = enemy.magicEndurance;
  ...
  if (enemy.meleeHeavyAttackEvasion) minified.mhae = enemy.meleeHeavyAttackEvasion;
  if (enemy.rangedHeavyAttackEvasion) minified.rhae = enemy.rangedHeavyAttackEvasion;
  if (enemy.magicHeavyAttackEvasion) minified.mghae = enemy.magicHeavyAttackEvasion;
  ...
  if (enemy.criticalDamageResistance) minified.cdr = enemy.criticalDamageResistance;
...
export function serializeState(state: Partial<AppState>): string {
  ...
  if (state.xAxisStat && state.xAxisStat !== "meleeEndurance") minified.x = state.xAxisStat;
  if (state.xAxisRange && (state.xAxisRange.min !== 0 || state.xAxisRange.max !== 3000 || state.xAxisRange.step !== 100)) {
    minified.xr = [state.xAxisRange.min, state.xAxisRange.max, state.xAxisRange.step];
  }
  if (state.yMetric && state.yMetric !== "expectedDamage") minified.y = state.yMetric;
  if (state.combatType && state.combatType !== "melee") minified.c = state.combatType;
  if (state.attackDirection && state.attackDirection !== "front") minified.ad = state.attackDirection;
  ...
  if (state.skillConfig) {
    const sc: any = {};
    if (state.skillConfig.skillPotency !== 1.0) sc.p = state.skillConfig.skillPotency;
    if (state.skillConfig.skillFlatAdd !== 0) sc.f = state.skillConfig.skillFlatAdd;
    if (state.skillConfig.hitsPerCast !== 1) sc.h = state.skillConfig.hitsPerCast;
...
export function deserializeState(hash: string): Partial<AppState> | null {
  try {
    ...
    if (minified.xr && Array.isArray(minified.xr) && minified.xr.length === 3) {
      state.xAxisRange = { min: minified.xr[0], max: minified.xr[1], step: minified.xr[2] };
    }
