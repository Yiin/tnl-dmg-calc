...
export const DamageFormula: React.FC<DamageFormulaProps> = ({
  ...
  const getCombatStats = () => {
    ...
    return {
      ...
      heavyAttack: build[`${prefix}HeavyAttack` as keyof Build] as number || 0,
      heavyEvasion: enemy[`${prefix}HeavyAttackEvasion` as keyof Enemy] as number || 0,
      ...
      <CardContent className="space-y-4">
        <div className="text-xs font-mono space-y-2 overflow-x-auto">
          ...
          <div className="whitespace-pre">
            ...
            <br />
            HeavyChance = ({totalHeavy}{heavyMod > 0 ? ` [${stats.heavyAttack}+${heavyMod}]` : ''}) / ({totalHeavy} + {stats.heavyEvasion} + 1000) = <span className="text-purple-500">{heavyChance}</span>
            <br />
            ExpectedHeavyMultiplier = (HeavyChance × 2) + ((1 - HeavyChance) × 1) = <span className="text-purple-500">{(parseFloat(heavyChance) * 2 + (1 - parseFloat(heavyChance)) * 1).toFixed(3)}</span>
          </div>
          ...
          <div className="whitespace-pre">
            ...
            <br />
            DefenseReduction = 1 - {stats.defense} / ({stats.defense} + 2500) = <span className="text-primary">{defenseReduction}</span>
            <br />
            DamageReduction = 1 - {enemy.damageReduction || 0}/100 = <span className="text-primary">{damageReduction}</span>
            <br />
            SkillDamageReduction = 1 - {enemy.skillDamageResistance || 0}/100 = <span className="text-primary">{skillResist}</span>
          </div>

          <div className="whitespace-pre">
            <span className="text-muted-foreground">// Skill Damage Boost</span>
            <br />
            SkillDamageBoost = {build.skillDamageBoost || 0} / 100 = <span className="text-primary">{((build.skillDamageBoost || 0) / 100).toFixed(3)}</span>
          </div>

          <div className="whitespace-pre">
            <span className="text-muted-foreground">// Bonus Damage & Damage Reduction (applied at end)</span>
            <br />
            BonusDamage = <span className="text-primary">{build.bonusDamage || 0}</span>
            <br />
            DamageReduction = <span className="text-primary">{enemy.damageReduction || 0}</span>
          </div>

          <div className="whitespace-pre">
            <span className="text-muted-foreground">// {isPvP ? 'PvP' : 'PvE'} Damage Multiplier</span>
            <br />
            {isPvP ? 'PvPMultiplier' : 'PvEMultiplier'} = {isPvP ? '1 - 0.1 = 0.9' : '1.0'} <span className="text-muted-foreground">({isPvP ? '10% damage reduction in PvP' : 'No reduction in PvE'})</span>
          </div>

          <div className="whitespace-pre border-t pt-2">
            <span className="text-muted-foreground">// Final Damage Formula (per source)</span>
            <br />
            <span className="text-yellow-500">Damage</span> = ((((SkillPotency × BaseDamage) + SkillFlatAdd) × 
            <br />
            <span className="ml-4">[DefenseReduction × CritDamageMultiplier × SkillDamageBoost ×</span>
            <br />
            <span className="ml-4">{isPvP ? 'PvPMultiplier' : 'PvEMultiplier'}]) × HeavyAttack) +</span>
            <br />
            <span className="ml-4">BonusDamage - DamageReduction</span>
            <br />
            <br />
            <span className="text-muted-foreground">// Expected Damage Per Cast (accounting for hit chance)</span>
            <br />
            <span className="text-yellow-500">ExpectedDamage</span> = Damage × HitChance × HitsPerCast
          </div>
          ...
          {build.offhandChance && build.offhandChance > 0 && (
            <div className="whitespace-pre border-t pt-2">
              ...
              <br />
              OffhandDamage = ({build.offhandMinDMG} + {build.offhandMaxDMG}) / 2 × {build.offhandChance} = <span className="text-primary">{((build.offhandMinDMG || 0) + (build.offhandMaxDMG || 0)) / 2 * build.offhandChance}</span>
              <br />
              <span className="text-yellow-500">TotalExpectedDamage</span> = ExpectedDamage + (OffhandDamage × AllMultipliers)
            </div>
          )}
        </div>
      </CardContent>
